...
optimParamsContinuous <- function(data, family, lower, upper, defaults,
  ...
  optim_progress <- data.frame(matrix(
    nrow = 0,
  ...
  for (i in 1:n_starting_points) {
    ...
    loglik_fun <- loglik(
      family = family, data = data, fixed = fixed,
    ...
    if (optim_result$convergence != 0 && debug_error) {
      print(tail(optim_progress, 2))
      warning(
        "No convergence in first optimization for family ",
        family$family
      )
    }
  ...
  if (nrow(optim_progress) > 0 &&
    ...
    if (debug_error) {
      message(
        "Final Optimization result for family ", family$family,
