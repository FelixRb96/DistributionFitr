## Authors Tim Glockner, tiglockn@mail.uni-mannheim.com Adrian
## Heppeler, aheppele@mail.uni-mannheim.de Borui Niklas Zhu,
## bzhu@mail.uni-mannheim.de Extract distribution families
## along with infos regarding their parameters from multiple R
## packages Copyright (C) 2019 -- 2020 Tim Glockner, Adrian
## Heppeler, Borui Niklas Zhu This program is free software;
...
iterate_packages <- function(packages) {
  ...
  for (i in 1:length(packages)) {
    ...
    message(
      "\tNumber of families:", length(package_content),
      "\n"
    )

    ## no distribution family contained in package
    if (length(package_content) == 0) {
      next
    }
    ...
    for (j in 1:length(package_content)) {
      ...
      message(
        "Current Family:", package_content[[j]]$family,
        "\n"
      )
      ...
      params <- tryCatch(getParams(package_content[[j]]),
        error = function(e) {
          message(
            "Error occured for family ", package_content[[j]]$family,
            "\n", e
          )
          NULL
        }
...
construct_package_list <- function(all.packages) {
  if (all.packages) {
    ...
    package_filter <- ((ins_pck[, "Priority"] == "base" |
      ins_pck[, "Priority"] == "recommended") & !is.na(ins_pck[
      ,
      "Priority"
    ]))
...
write_file <- function(FamilyList, file = "R/all_families.R") {
  for (i in 1:length(FamilyList)) {
    for (j in 1:length(FamilyList[[i]]$family_info)) {
      ...
      if (is.double(ij)) {
        FamilyList[[i]]$family_info[[j]] <- signif(
          ij,
          5
        )
      }
...
### Case 1: all.packages given: search for distribution
### families Case 1.1 vector of strings (package names) -> take
### families from those packages Case 1.2 FALSE -> only take
### recommended / base packages Case 1.3 TRUE -> take all
### installed packages Case 2 all.packages missing: Take
### families saved in the file

getFamilies <- function(all.packages) {

  ## CASE 2: -> Take families as saved in internal variable
  ## FamilyList
