...
globalfit <- function(data, continuity = NULL, method = "MLE", verbose = TRUE,
  ...
  if (continuity) {
    ...
    if (length(discrete_families) > 0) {
      ...
      stop(
        "Data is assumed to be discrete but there are no discrete distributions",
  ...
  if (is.null(cores)) {
    ...
    if (length(CRAN_check_limit) > 0 && CRAN_check_limit == TRUE) {
      cores <- 2
  ...
  output_liste <- foreach(
    ...
    if (!is.null(result_optim) && !is.na(result_optim$value) &&
      ...
      if (do_sanity) {
        sanity_check <- list(
          hist_check = NA, int_check = NA, L1_check = NA,
          good = FALSE
        )
        output@sanity <- sanity_check
      }
    ...
    if (debug) {
      cat(
        "Elapsed time for family", fam$family, "from package", fam$package,
