...
get_all_params <- function(fam) {
  # idea: all params need to be present in the r... 
  # method for generating random samples from the distribution
...
validate_values <- function(fam, n_or_nn, params, x_test) {
  ...
  if (is.finite(r)) {
    ...
    if (r_ == "TIMEOUT") message(fam, " produced timeout for params ", 
...
get_param_ranges <- function(all_params, fam) {
  ...
  for (param in names(all_params)){
    ...
    # if lowest or highest value was valid in the first check we already 
    # have an min_val or max_val
    # otherwise we iterate with the above method
    ## MS min_val <- if () ...
    if (check_res[1]) {
      min_val <- - Inf
    } else {
      min_val <- iterate_min_max_vals(param=param, all_params = all_params, 
                                      fam=fam, cur_val = min(vals[check_res]), 
    ...
    if(accepted_float_rate > 1/(num_tests - num_integer)) 
      accepts_float[param] <- TRUE
    else if(accepted_int_rate == 0) 
      stop("distribution does not seem to accept any values")
...
get_support <- function(fam, params) {
  ...
  # remark: what happens with values x: lox_capped < x < 
  #            low_capped + 0.1 * (upp_capped - low_capped), i.e. 0.05?
  ## MS keine Konstanten mitten im Code  
  low <- ifelse(params$lower == low_capped, low_capped + 
                  0.1 * (upp_capped-low_capped), low_capped)
  upp <- ifelse(params$upper == upp_capped, upp_capped - 
                  0.1 * (upp_capped-low_capped), upp_capped)
  ...
  # base_choices <- as.list(ifelse(params$accepts_float, base_choices, 
  #                         round(base_choices)))
