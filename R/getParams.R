...
validate_values <- function(fam, n_or_nn, params, x_test) {
  ...
  if (is.finite(r)) {
    r_ <- eval_with_timeout(do.call(dfun, c(x_test, params)),
      timeout = 1,
    ...
    if (any(r_ == "TIMEOUT")) {
      message(
        fam, " produced timeout for params ",
        paste(names(params), params,
          sep = ": ",
          collapse = ","
        ), " on ", x_test
      )
