...
setMethod(f = "sort", signature = c("globalfit"), def = function(x, decreasing = FALSE,
                                                                 ic = c("BIC", "AIC", "AICc")) {
  ic <- match.arg(ic)
...
setMethod(f = "summary", signature = c("globalfit"), def = function(object, n = 10,
                                                                    ic = c("BIC", "AIC", "AICc")) {
  ic <- match.arg(ic)

  if (length(object@fits) < 1) {
    warning(
      "No family fitted. Either packages provided in ", "argument 'packages' do not supply reasonable parametric",
    ...
    df <- data.frame(family = sapply(object@fits, function(f) f@family), package = sapply(
      object@fits,
      function(f) {
        f@package
      }
    ), ic = sapply(object@fits, function(f) f %@% ic), params = sapply(
      object@fits,
      function(f) {
        paste(names(f@estimatedValues), signif(f@estimatedValues, digits = 3),
          sep = " = ", collapse = "; "
        )
      }
    ))
  ...
  return(new("globalfitSummary",
    call = object@call, data = object@data, continuity = object@continuity,
    method = object@method, fits = df, ic = ic
  ))
})

setMethod(f = "show", signature = c("globalfitSummary"), def = function(object) {
  cat("\nCall: \n", object@call, "\n\n")
  ...
  cat(
    length(object@data), "data points entered. Distributions were fitted via",
    object@method, "estimation.", cont, "
                \nBest fits sorted by",
    object@ic, ":\n\n"
  )
...
setMethod(f = "hist", signature = c("globalfit"), def = function(x, which = 1, ic = c(
  ...
  # first create the histogram without plotting, because we need its maximum
  # density for setting ylim
  h <- hist(x = x@data, breaks = breaks, plot = FALSE)

  plot(h, xlim = range(lower, upper), xlab = "x", freq = FALSE, ylim = range(
    0,
    max(h$density), max(density)
  ), ylab = "density", main = paste0(
  ...
  if (!x@continuity) {
    points(supporting_point, density, col = "green", lwd = 2)
  }
