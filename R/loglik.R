## Authors Niclas Lietzow, nlietzow@mail.uni-mannheim.de Till Freihaut,
## tfreihau@mail.uni-mannheim.de Leonardo Vela, lvela@mail.uni-mannheim.de Define
## the log-likelihood function for given parameters Copyright (C) 2019 -- 2020
## Niclas Lietzow, Till Freihaut and Leonardo Vela This program is free software;
...
loglik <- function(family, data, fixed = list(), log, lower, upper) {
  ...
  # define loglikelihood function BNZ: allow for empty params for distributions
  ...
  likelihood <- function(params = NULL) {
    for (param_name in names(params)) {
      if (lower[param_name] > params[[param_name]] || upper[param_name] < params[[param_name]]) {
        stop(
          "Parameter ", param_name, " with value ", params[[param_name]],
    ...
    ## The following is only for tracking the optimisation progress (might be
    ## deactivated sometime) recursively go through parent frames and check whether
    ## there is a variable tracks the optimisation process
