# Loglik-Function important: family should be list with elements 'package' and
# 'family'
loglik <- function(param_values, family, data, fixed = list(), log = TRUE, show_optim_progress = FALSE) {
  ...
  if (show_optim_progress) {
    # recursively go through parent frames and check whether there is a variable that
    ...
    if (return_optim_progress) {
      ...
      # print(progress) print(c(param_values, fixed, log_lik = ll))
...
## Parameters of optimParam: family: list with two elements 'family' and 'package'
## lower, upper and start_parameters must only contain the continuous parameters
## that should be optimized prior: user-given prior information on parameters,
## updates default values from get_param debug_error: show optimization progress
## when an error occured show_optim_progress: always show optimization progress
## on_error_use_best_result: if TRUE and an error occured during optimization the
## best result achieved prior to the error will be taken
optimParam_plot <- function(plot = TRUE, dim1 = NULL, dim2 = NULL, data, family,
                            lower, upper, defaults, method = "MLE", fixed = NULL, prior = NULL, log = TRUE,
                            optim_method = "L-BFGS-B", debug_error = TRUE, show_optim_progress = FALSE, on_error_use_best_result = TRUE,
                            ...) {
  # Input parameter validation if(length(fixed) > 0) { stop('Fixed parameters not
  # allowed for this testing function.') }
  if (method != "MLE") {
  ...
  if (length(prior) > 0) {
    ...
    # nomatch should not occur anyway due to the check above: 'Parameter names given
    # as prior unknown'
  ...
  if (plot == FALSE) {
    ret <- optimParam(
      data = data, family = family, lower = lower, upper = upper,
      defaults = defaults, method = "MLE", fixed = NULL, prior = NULL, log = TRUE,
      optim_method = "L-BFGS-B", debug_error = TRUE, show_optim_progress = FALSE,
      on_error_use_best_result = TRUE, ...
    )
    return(ret)
  }

  # create dataframe where to save the optimization progress 1 column for each
  # parameter and a column for the associated log likelihood
  optim_progress <- data.frame(matrix(nrow = 0, ncol = length(lower) + length(fixed) +
    1))
  ...
  diagram <- NULL
  # get parameters to maximise over, partition their intervals and form all
    ...
    step <- tryCatch(
      {
        ...
        optim_result <- optim(inits[i, ], loglik,
          family = family, data = data,
          fixed = fixed, lower = lower, upper = upper, log = log, control = list(
            fnscale = -1,
            trace = 0, maxit = 1
          ), method = optim_method
        )
  ...
  gg <- ggplot(data = diagram) +
    geom_segment(aes_string(x = dim1, y = dim2, xend = paste0(
      dim1,
      "_it"
    ), yend = paste0(dim2, "_it")), arrow = arrow(length = unit(0.2, "cm"))) +
    geom_segment(x = max(lower[dim1], -100), y = max(lower[dim2], -100), xend = min(
      upper[dim1],
      100
    ), yend = max(lower[dim2], -100), arrow = arrow(length = unit(
      0.2,
      "cm"
    )), col = "green") +
    geom_segment(x = max(lower[dim1], -100), y = max(
      lower[dim2],
      -100
    ), xend = max(lower[dim1], -100), yend = min(upper[dim2], 100), arrow = arrow(length = unit(
      0.2,
      "cm"
    )), col = "green") +
    xlab(dim1) +
    ylab(dim2)
  if (length(fixed) > 0) {
    gg <- gg + ggtitle(paste("Konvergenzverhalten; konstante Parameter:", paste(names(fixed),
      fixed,
      sep = " = ", collapse = "; "
    )))
  } else {
...
diagram <- optimParam_plot(
  dim1 = "mean", dim2 = "sd", data = data, family = family,
  lower = lower, upper = upper, defaults = defaults, fixed = fixed, log = T, parscale = TRUE,
  fnscale = TRUE, show_optim_progress = TRUE
)


# Example 2: beta (three parameters) TODO: Find out where the instability w.r.t.
# ncp comes from
...
diagram <- optimParam_plot(
  dim1 = "ncp", dim2 = "shape1", fixed = c(shape2 = 10),
  data = data, family = family, lower = lower, upper = upper, defaults = defaults,
  log = T, show_optim_progress = F
)
diagram <- optimParam_plot(
  dim1 = "ncp", dim2 = "shape2", fixed = c(shape1 = 10),
  data = data, family = family, lower = lower, upper = upper, defaults = defaults,
  log = T, show_optim_progress = F
)
diagram <- optimParam_plot(
  dim1 = "shape1", dim2 = "shape2", fixed = c(ncp = 20),
  data = data, family = family, lower = lower, upper = upper, defaults = defaults,
  log = T, show_optim_progress = F
)

# Example 3: f-distribution (also three parameters) see if optim fails in three
# dimensions or whether the problem is specific w.r.t. ncp in beta
...
fixed <- list()
diagram <- optimParam_plot(
  dim1 = "ncp", dim2 = "df1", fixed = c(df2 = 10), data = data,
  family = family, lower = lower, upper = upper, defaults = defaults, log = T,
  show_optim_progress = F
)
